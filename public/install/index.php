...
$config = [
    ...
    'sqlName'       => 'kbctm3',             // 安装的sql文件名，同该文件同级的sql文件名相同
    'tableName'     => 'ykbtest_',                  // 安转的sql文件中的表名前缀，该项是被用户输入替换的
...
if ($get == $config['endPage']) {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $salt='4MzHJo';
        $md5password=md5(md5($_POST['password']) . md5($salt));
        $username=$_POST['username'];
        $sql="UPDATE ".$data['DB_PREFIX']."admin_user SET username='$username',password='{$md5password}',salt='{$salt}' WHERE id = 1 ";
        $link->query($sql);
        $link->close();
        $db_str = <<<php
<?php
...
return [
    ...
    // 服务器地址
    'hostname'        => Env::get('DB_HOST','{$data['DB_HOST']}'),
    // 数据库名
    'database'        => Env::get('DB_DATABASE','{$data['DB_NAME']}'),
    // 用户名
    'username'        => Env::get('DB_USERNAME','{$data['DB_USER']}'),
    // 密码
    'password'        => Env::get('DB_PASSWORD','{$data['DB_PWD']}'),
    // 端口
    'hostport'        => Env::get('DB_PORT','{$data['DB_PORT']}'),
    // 连接dsn
    'dsn'             => '',
    // 数据库连接参数
    'params'          => [],
    ...
    'charset'         => 'utf8',
    // 数据库表前缀
    'prefix'          => Env::get('DB_PREFIX','{$data['DB_PREFIX']}'),
    // 数据库调试模式
    'debug'           => true,
