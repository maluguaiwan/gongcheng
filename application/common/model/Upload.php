...
class Upload extends ModelBase
{
    ...
    public function upfile($type, $filename = 'file', $isthumb = false, $is_water = false,$id=0,$module='admin',$use='admin')
    {
        ...
        if (empty($n)) {
            ...
            $validate['ext'] = 'jpg,jpeg,gif,png,bmp,rar,zip';
            ...
            if ($sysconfig['open_7niu']) {
                ...
                if ($info) {
                   // Log::record('savename ' . $info->getSaveName(), 'info');
                    ...
                    $data['user_id'] = $id;
                    $data['use'] = $use;
                    if(!empty($data['module']) && $data['module'] == 'admin') {
                        //通过后台上传的文件直接审核通过
                        $data['status'] = 1;
                    ...
                    $res = $filemode->getLastInsID();
                    if ($res > 0) {
                        return array('code' => 200, 'msg' => '上传成功', 'hasscore' => 0, 'ext' => $data['ext'], 'id' => $res, 'path' => $path, 'headpath' => $realpath,'url'=>$url, 'md5' => $data['md5'], 'savename' => $info->getSaveName(), 'filename' => $info->getFilename(), 'info' => $info->getInfo());
                    } else {
                        return array('code' => 0, 'msg' => '上传失败');
                    }
                } else {
            ...
            $realpath = $path;
            $url=$n['savepath'];
            return array('code' => 200, 'msg' => '上传成功', 'hasscore' => 1, 'ext' => $n['ext'], 'id' => $n['id'], 'path' => $path, 'headpath' => $realpath,'url'=>$url, 'md5' => $md5, 'savename' => $n['savename'], 'filename' => $n['name'], 'info' => $n);
        }
