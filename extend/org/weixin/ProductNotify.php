...
class ProductNotify extends WxPayNotify {
	...
	public function NotifyProcessor($data, &$msg, &$log) {
	    $order = db('order')->where (['order_number' => $data['out_trade_no']])->find();
	    if (! $order) {
	        $msg = "订单不存在";
	        $log['status'] = 0;
	        $log['errmsg'] = $msg;
	    ...
	    if ($order) {
	        if(!db('wxpay_log')->where(['out_trade_no' => $data['out_trade_no']])->value('id')){
	            ...
	            Log::write('wxpay_log_id:'.$insert);
	                // 处理订单
	            $row = db('order')->where(['id'=>$order['id']])->update(['status'=>1,'pay_status'=>3,'update_time'=>time()]);
	            if($row !== false){
	            	// TODO 发放奖励
	            	reward(1,$order['id']);
	            }
